...
: "${TEMPO_REPO:?set TEMPO_REPO to a tempoxyz/tempo checkout}"
...
docker run -d --name "$CONTAINER_NAME" \
  ...
  "$TEMPO_IMAGE" \
  node --dev --chain=/tmp/tempo-genesis.json --dev.block-time=1s \
...
docker run --rm \
  ...
  -v "$EEST_REPO:/work" \
  -v "$SCRIPT_DIR:/benchmarkoor-tempo:ro" \
...
export_args=(
  ...
  --generator "EEST execute remote + Benchmarkoor Tempo adapter/exporter"
...
"$SCRIPT_DIR/export-suite.py" "${export_args[@]}"
